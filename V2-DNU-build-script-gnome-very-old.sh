...
sub-enable-os-controlled-networking ()
{
 ...
 if [ "$choice" = "n" ]
	...
	sudo wget https://cloudtechlinks.com/V23-cloudtech-dot-yaml  --output-document=/etc/netplan/v47-cloudtech-youtube-video.yaml
fi
}
...
sub-configue-nomachine-user ()
{
 ...
 if [ "$choice" = "n" ]
    ...
    sudo usermod -aG sudo,adm,ipadmin nomachine
