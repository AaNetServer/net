...
sub-enable-os-controlled-networking ()
{
 ...
 if [ "$choice" = "n" ]
	...
	sudo wget https://raw.githubusercontent.com/AaNetServer/net/refs/heads/main/v1-aaserver.yaml  --output-document=/etc/netplan/v1-aaserver.yaml
fi
}
...
sub-configue-nomachine-user ()
{
 ...
 if [ "$choice" = "n" ]
    ...
    sudo usermod -aG sudo,adm,lp,sys nomachine
